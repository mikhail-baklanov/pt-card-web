...
public class SystemRequest  extends com.google.gwt.http.client.Request {
	  ...
	  private static Response createResponse(final XMLHttpRequest xmlHttpRequest) {
	    ...
	    Response response = new Response() {
	      ...
	      public String getHeader(String header) {
	        SystemRequestBuilder.throwIfEmptyOrNull("header", header);
	      ...
	      public Header[] getHeaders() {
	        return SystemRequest.getHeaders(xmlHttpRequest);
	      }
	  ...
	  /**
	   * Returns an array of headers built by parsing the string of headers returned by the JavaScript
	  ...
	  /*
	   * JavaScript XmlHttpRequest object that this Java class wraps. This field is not final because we transfer ownership
	   * of it to the HTTPResponse object and set this field to null.
	   */
	   ...
	   *           if xmlHttpRequest, or callback are null
	   */
	  SystemRequest(XMLHttpRequest xmlHttpRequest, int timeoutMillis, final RequestCallback callback) {
	  ...
	  public void cancel() {
	    /*
	     * There is a strange race condition that occurs on Mozilla when you cancel a request while the response is coming
	     * in. It appears that in some cases the onreadystatechange handler is still called after the handler function has
	     * been deleted and during the call to XmlHttpRequest.abort(). So we null the xmlHttpRequest here and that will
	     * prevent the fireOnResponseReceived method from calling the callback function. Setting the onreadystatechange
	     * handler to null gives us the correct behavior in Mozilla but crashes IE. That is why we have chosen to fixed this
	     * in Java by nulling out our reference to the XmlHttpRequest object.
	     */
	    if (xmlHttpRequest != null) {
	  ...
	  public boolean isPending() {
	    ...
	    /*
	     * Because we are doing asynchronous requests it is possible that we can call XmlHttpRequest.send and still have the
	     * XmlHttpRequest.getReadyState method return the state as XmlHttpRequest.OPEN. That is why we include open although
	     * it is nottechnically true since open implies that the request has not been sent.
	     */
	    switch (readyState) {
	  ...
	  void fireOnResponseReceived(RequestCallback callback) {
	    ...
	    /*
	     * We cannot use cancel here because it would clear the contents of the JavaScript XmlHttpRequest object so we
	  ...
	  /*
	   * Method called when this request times out. NOTE: this method is called from JSNI
	   */
	  ...
	  /**
	   * Tests if the JavaScript <code>XmlHttpRequest.status</code> property is readable. This can return failure in two
	   * different known scenarios:
	   * <ol>
	   * <li>On Mozilla, after a network error, attempting to read the status code results in an exception being thrown. See
	   * <a href="https://bugzilla.mozilla.org/show_bug.cgi?id=238559"
	   * >https://bugzilla.mozilla.org/show_bug.cgi?id=238559</a>.</li>
	   * <li>On Safari, if the HTTP response does not include any response text. See <a
	   * href="http://bugs.webkit.org/show_bug.cgi?id=3810">http://bugs.webkit.org /show_bug.cgi?id=3810</a>.</li>
	   * </ol>
	   * 
	   * @param xhr
	   *          the JavaScript <code>XmlHttpRequest</code> object to test
	   * @return a String message containing an error message if the <code>XmlHttpRequest.status</code> code is unreadable
	   *         or null if the status code could be successfully read.
	   */
